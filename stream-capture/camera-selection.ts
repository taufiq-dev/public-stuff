...
// Escape hatch populated from field reports: a rear candidate whose label contains one of these
// fragments wins outright, bypassing the heuristics.
...
export const selectMainRearCamera = async (
  ...
  if (useCache) {
    ...
    if (match) {
      return finish(toCandidate(match, readDeviceInfoCapabilities(match), "device-info"), [], "cached");
    }
  }
  ...
  if (devices.length <= 1) {
    ...
    const camera = toCandidate(device, readTrackCapabilities(defaultTrack), "default-stream");
    return {
      stream: defaultStream,
      camera,
      allCameras: [{ ...camera, score: scoreCamera(camera), isRear: true }],
  ...
  const cheap = devices.map((device) => {
  ...
  // 6. Rank.
  const scored: ScoredCandidate[] = candidates.map((camera) => ({
    ...camera,
    isRear: isRearCamera(camera),
    score: scoreCamera(camera),
  }));
  const rear = scored.filter((c) => c.isRear).sort((a, b) => b.score - a.score);
