    ...
    async (open: () => Promise<CameraSelectionResult>) => {
      ...
      try {
        ...
        result.stream.getVideoTracks()[0]?.addEventListener("ended", () => {
          if (runId === runRef.current) {
            setState({ status: "error", message: "The camera was interrupted. Please try again." });
          }
        });
        ...
        if (runId === runRef.current) setState({ status: "error", message: describeCameraError(error) });
      }
    },
    [releaseStream],
  );
